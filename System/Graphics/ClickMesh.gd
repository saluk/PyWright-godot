...
var original_mesh
var click_uv = preload("res://System/Graphics/click_uv.gdshader")
...
func make_click_mesh():
	for surf in range(mesh.get_surface_count()):
		...
		for i in range(original_mesh.regions.size()):
			shader_mat.set_shader_parameter("region"+str(i+1), original_mesh.regions[i])
		shader_mat.set_shader_parameter("region_max", original_mesh.regions.size())
